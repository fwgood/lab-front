  ...
  renderComment() {
    ...
    return (
      <List
        ...
        renderItem={item => (
          <List.Item
            actions={[<a onClick={()=>this.replay(item)} href="#replay">回复</a>]}
            extra={
              <div className={styles.subComment}>
                ...
                <div className={styles.subCommentLay}>
                  <div>gauthier<span style={{float:"right"}}>2018-12323</span></div>
                  <div className={styles.subCommentContent}>Ant Desigvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvn, a design</div>
                </div>
              </div>
            }
          >
            <Skeleton avatar title={false} loading={item.loading} active>
              <List.Item.Meta
                ...
                description="Ant Design, a design language for background applicationdsddddddddddddddddddddddddddddddddddddds, is refined by Ant UED Team"
              />
              <div>时间</div>
            </Skeleton>
          </List.Item>
  ...
  renderMyDiscuss() {
    ...
    return (
      <Card
        ...
        title={<a style={{ fontSize: 22 }}>{'我是标题'}</a>}
      >
          ...
          <div className={styles.description}>
            <div className={styles.extra}>
              <Avatar
                ...
                size="small"
              />
              <a>{'  ' + this.state.blog.name}</a> 发布在
              <a>{' 未知'}</a>
              <em style={{ marginLeft: 5 }}>
                {moment(this.state.blog.time).format('YYYY-MM-DD HH:mm')}
              </em>
            </div>
            <div className={styles.contentF}>{this.state.blog.content + 'dddddddddddddd'}</div>
        ...
        <Row>
          <div>
            <div className={styles.commentInput}>
              <Input.TextArea id="replay" placeholder={this.state.currentOperation== '' ? "留下你的观点":this.state.currentOperation} autosize={{ minRows: 2, maxRows: 6 }} />
